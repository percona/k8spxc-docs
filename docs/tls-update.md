...
2. Create a Secret object and provide the new certificates within it. The Secret name **must be in the format `<existing-secret>-new`**.
   ...
   The Operator automatically detects Secrets only if their names end with `-new`. Any Secret that does not have `-new` at the end of its name will be ignored and will **not** trigger a certificate update.  
