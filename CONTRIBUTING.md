# Contributing to Percona Operator for MySQL based on Percona XtraDB Cluster documentation

This repository contains the source files for Percona Operator for MySQL based on Percona XtraDB Cluster documentation and this document explains how you can contribute to it.

If you'd like to submit a code patch, follow the [Contributing guide in the Operator code repository](https://github.com/percona/percona-xtradb-cluster-operator/blob/main/CONTRIBUTING.md).
...
5. Select the Percona Operator for MySQL based on Percona XtraDB Cluster in the Project dropdown and the issue type in the Issue Type dropdown. Click Next.
...
Percona Operator for MySQL based on Percona XtraDB Cluster documentation is written in [Markdown](https://www.markdownguide.org/basic-syntax/) language, so you can
...
3. Go to `k8spxc-docs` and add the remote upstream repository:

    ```sh
    git remote add upstream git@github.com:percona/k8spxc-docs.git
    ```
...
5. Create a branch for the changes you are planning to make. If there is a Jira ticket related to your contribution, name your branch in the following way: `<Jira issue number>-<short description>`, where the issue number is something like `K8SPXC-372`.

   Create the branch in your local repo as follows:

   ```
   git checkout -b K8SPXC-372-fix-feature-X
   ```
...
6. When your changes are ready, make a commit, mentioning the Jira issue in the commit message, if any:
   ...
   ```
   git add .
   git commit -m "K8SPXC-372 fixed by ......"
   git push -u origin K8SPXC-372-fix-feature-X
   ```
   ...
   ```
   remote: Create a pull request for 'K8SPXC-372-fix-feature-X' on GitHub by visiting:
   remote:      https://github.com/<your-name>/k8spxc-docs/pull/new/K8SPXC-372-fix-feature-X
   ```
